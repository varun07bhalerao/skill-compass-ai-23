...
const Login = () => {
  const { t } = useLanguage();
  const { user, login, signup, loginWithGoogle, isAuthenticated } = useAuth();
  ...
  const checkProfileAndNavigate = async (userEmailToCheck: string) => {
    if (!userEmailToCheck) {
    ...
    try {
      const docRef = doc(db, "userProfiles", userEmailToCheck);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignup) {
      ...
      if (success) {
        ...
        navigate("/skill-profile"); // Force new users to the profile setup
      } else {
      ...
      if (success) {
        ...
        await checkProfileAndNavigate(email);
      } else {
  ...
  const handleGoogleLogin = async () => {
    const userGoogleEmail = await loginWithGoogle(); 
    
    if (userGoogleEmail) {
      toast.success("Logged in with Google!");
      await checkProfileAndNavigate(userGoogleEmail);
    } else {
